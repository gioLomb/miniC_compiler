...
static ASTNode *optimizeStmt(ASTNode *stmt) {
    ...
    case ND_BLOCK: {
        /* compatta l'array children scartando gli statement per cui
           optimizeStmt ha restituito NULL (rimossi) */
        int writeIdx = 0;
        for (int i = 0; i < stmt->nchildren; i++) {
            ASTNode *result = optimizeStmt(stmt->children[i]);
            if (result) {
                stmt->children[writeIdx++] = result;
            }
        }
        stmt->nchildren = writeIdx;
        return stmt;
    }
